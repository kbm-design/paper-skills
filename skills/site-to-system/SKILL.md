...
### 3. Create the tokens
`create_tokens` for the Canonical set, **in the order Paper wants**: semantic colors before palette colors, neutrals first, then primary → secondary → accent; every other type smallest-value-first. Use `var(--…)` aliases so semantic tokens point at palette tokens (change the palette once, the whole system moves). Put Candidates on the proposals board instead — do not create them.
